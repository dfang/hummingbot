FROM ghcr.io/prefix-dev/pixi:0.39.5 AS builder

# install system dependencies
RUN apt-get update && \
    apt-get install -y git sudo libusb-1.0 gcc g++ python3-dev && \
...
RUN echo "#!/bin/bash" > /home/hummingbot/entrypoint.sh
RUN cat /shell-hook >> /home/hummingbot/entrypoint.sh
RUN echo 'exec "$@"' >> /home/hummingbot/entrypoint.sh
...
FROM ghcr.io/prefix-dev/pixi:0.39.5 AS release
...
CMD pixi run ./bin/hummingbot_quickstart.py 2>> ./logs/errors.log
